...
export class CareerFormComponent implements OnInit {
  ...
  solutionForm: FormGroup = new FormGroup({
    ...
    User_Contact_Number: new FormControl('',[ Validators.pattern("^((\\+91-?)|0)?[0-9]{10}$")]),
  });
...
districtData.push(item.District)
  ...
  public onClickNext(): void {
    ...
    if (this.solutionForm.valid) {
      const profileFormValue= JSON.parse(localStorage.getItem('profileFormValue') as any);
      const studentDetail = JSON.parse(localStorage.getItem('studentDetail') as any);
